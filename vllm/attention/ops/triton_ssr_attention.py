...
@triton.jit
def kernel_ssr_unified_attention_2d(
    ...
    attn_scores_ptr,  # [num_query_tokens, num_query_heads, max_seqlen] - NEW
    attn_scores_stride_0: tl.int64,  # int - NEW (stride for query token dim)
    attn_scores_stride_1: tl.int64,  # int - NEW (stride for head dim)
    max_seqlen: tl.constexpr,  # int - NEW (max seqlen dimension size)
    ...
    for j in range(0, num_blocks):
        ...
        # Compute offsets for storing attention scores
        # attention_scores_ptr: [num_query_tokens, num_query_heads, max_seqlen]

        # Base offset for each query in BLOCK_M
        attn_base_offset = (query_offset_0[:, None] * attn_scores_stride_0 +
                            query_offset_1[:, None] * attn_scores_stride_1)

        # Add sequence offset for the keys
        attn_offset = attn_base_offset + seq_offset[None, :]

        # Create mask for valid stores
        # - query_mask_0: valid query tokens (not padding)
        # - query_mask_1: valid query heads (not out of bounds)
        # - seq_offset < max_seqlen: within allocated tensor bounds
        store_mask = (query_mask_0[:, None] &
                      query_mask_1[:, None] &
                      (seq_offset[None, :] < max_seqlen))

        # Store the attention scores
        tl.store(
            attn_scores_ptr + attn_offset,
            S,
            mask=store_mask
        )
...
@triton.jit
def kernel_ssr_unified_attention_3d(
        segm_output_ptr,
        ...
        attn_scores_ptr,  # [num_query_tokens, num_query_heads, max_seqlen]
        attn_scores_stride_0: tl.int64,  # int
        attn_scores_stride_1: tl.int64,  # int
        max_seqlen: tl.constexpr,  # int
):
    q_block_global_idx = tl.program_id(0)
    kv_head_idx = tl.program_id(1)
...
def ssr_unified_attention(
    ...
    if max_seqlen_q > 1 or total_num_q_blocks * num_kv_heads > 128:
        kernel_ssr_unified_attention_2d[(
            total_num_q_blocks,
            num_kv_heads,
        )](
            output_ptr=out,
            ...
            attn_scores_stride_0=attn_scores.stride(0),
            attn_scores_stride_1=attn_scores.stride(1),
            max_seqlen=attn_scores.shape[2],
        )
    else:
        ...
        kernel_ssr_unified_attention_3d[(
    ...
    focus_len = max(max_seqlen_k, topk)
